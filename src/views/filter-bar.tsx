...
const FilterButton = styled.button<{ active: boolean }>`
 padding: ${(p) => p.theme.spacing(1)} ${(p) =>
p.theme.spacing(2)};
 border: 1px solid
 ${(p) => (p.active ? p.theme.colors.accent :
p.theme.colors.border)};
border-radius: ${(p) => p.theme.radius.md};
 background: ${(p) =>
 p.active ? p.theme.colors.accent :
...
p.theme.colors.text)};
 ...
 border-color: ${(p) =>
 p.active ? p.theme.colors.accentHover :
p.theme.colors.border};
 }
`;
